...
use crate::{
    ...
    error::Error,
    protocol::{
        packets::clientbound::login::{disconnect::{Disconnect, DisconnectMessage}},
        varint::VarInt,
        vec::PrefixedVec,
        Processable
    },
...
impl Processable for EncryptionResponse {
    fn process(self, connection: &mut Connection) -> Result<(), Error> {
        ...
        //     properties,
        // };
        connection.write_packet(Disconnect {
            component: DisconnectMessage { text: "Unimplemented!".to_string() }
        })?;
